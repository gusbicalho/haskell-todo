...
@identity@. This field has the type @Extensible identityType@. The @Extensible@
type is serialized in such a way that a token with an identity field that does
not match the @identityType@ will not give an error, but return an @Other@
value.

In applications, tipically code should pattern match on
@(AuthTokenClaims (Known i))@, which ensures the token was parsed correctly and
has a payload with the expected shape.
-}
